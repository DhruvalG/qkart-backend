...
const getUser = catchAsync(async (req, res) => {
  ...
  // if(q){
  //   user = await userService.getUserByEmail(userId, q);
...
const setAddress = catchAsync(async (req, res) => {
  ...
  if(!user){
    throw new ApiError(httpStatus.getUserById(req.params.userId));
  }
  if(user.email != req.user.email){
    throw new ApiError(httpStatus.FORBIDDEN, "User not authorized to access this resource");
  }
  let address = await userService.setAddress(user, req.body.address);
  res.send({
    address: address,
  });
});
